...
model = YOLO("D:\\Automatic_Number_Plate_Recognition\\best.pt")
...
def real_time():
    lp = ""
    frm_nm = -1
    cap = cv2.VideoCapture("D:\\Automatic_Number_Plate_Recognition\\video.mp4")
    ret = True

    with open("results1.csv", mode='w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(['Frame_number', 'Timestamp', 'License_plate_text', 'License_plate_coordinates', 'Confidence_score'])
                        ...
                        for bbox, text, confidence in dat:
                            ...
                            if len(text) == 10:
                                if license_complies_format(text):
                                    ...
                                    license_plate = format_license(text)
                                    ...
                                    write_csv(csv_writer, frm_nm, timestamp, license_plate, coordinates, confidence)
...
def write_csv(csv_writer, frm_nm, time, text, coordinates, confidence):  # Write OCR text to CSV
    csv_writer.writerow([frm_nm, time, text, coordinates, confidence])
